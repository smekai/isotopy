...
**Priority:** P2
**Tags:** ui, server, setup
**Updated:** 2026-07-23 11:15

Follow-up from TASK-064. Engine, model, permission mode, pipeline and disabled stages are *conceptually* project settings but physically browser settings: [`packages/ui/src/settings.ts`](../packages/ui/src/settings.ts) keys them `adhd.<projectId>.<name>` in `localStorage`. Open the app in another browser (or clear site data) and a project silently reverts to defaults, while the folder, skills and credentials it belongs to are all durable server-side.

Move the non-secret preferences into the per-project section of `~/.adhd/settings.json` alongside the engine connection (`services/settings-store.ts` already has `defaults` + per-project overrides), expose them through `/settings`, and let the UI read/write them there. Secrets stay where TASK-059 put them — never inside a project folder. Decide what a first read does with an existing `localStorage` value: adopt it once, or ignore it.

**Verify:** a preference set in one browser is visible in another; `pnpm test` covers the store; `pnpm --filter @adhd/ui e2e` still passes (`ui-smoke.spec.ts` asserts persistence across a reload and reads the storage key directly — it will need rewriting against the API).

**Cross-platform:** n/a — no new paths beyond the existing user-level settings file.
