...
**Order:** `TASK-109` (done) → `TASK-110` → `TASK-120` → `TASK-112` → `TASK-114` → **`TASK-121`** (structural cleanup) → `TASK-117` (E2E). The refactor lands before the browser gate so the release verdict is recorded against the structure we intend to keep.
