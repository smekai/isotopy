...
**Order:** `TASK-109` (done) → `TASK-110` (done) → `TASK-120` (done) → `TASK-112` (done) → `TASK-114` (done, MVP slice) → **`TASK-121`** (structural cleanup) → `TASK-117` (E2E). The refactor lands before the browser gate so the release verdict is recorded against the structure we intend to keep.
