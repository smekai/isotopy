# MVP Scope: Artificial Developer
...
|--------|---------|-----------------|
| **Built-in task** | `adev run --task TASK-001` | Loads task from `.adev/tasks/`; links run to task |
| **Plain text idea** | `adev run "Add user login with email OTP"` | Creates run from string |
| **Markdown file** | `.adev/inputs/feature.md` | Loads title, body, optional acceptance criteria |
| **GitHub issue URL** | `adev run --issue https://github.com/org/repo/issues/42` | Fetches title + body via `gh` CLI (optional dep) |
...
Repo-native backlog stored under `.adev/tasks/` in the target repo. Tasks are the persistent planning layer; runs are the execution unit. A task can spawn one or more runs over time (e.g. initial implementation, then a follow-up iteration).
...
| Command | Behavior |
|---------|----------|
| `adev task create` | Create task (interactive or `--title`, `--description`) |
| `adev task list` | List tasks; filter by `--status`, `--priority`, `--tag` |
| `adev task show <id>` | Full task detail |
| `adev task move <id> --status <status>` | Change task status |
| `adev task update <id>` | Update title, description, priority, tags |
| `adev run --task <id>` | Start lifecycle run from task; sets `taskId` on run |
...
**Stage restart:** `adev restart <run-id> --from <stage-id>` re-runs from that stage forward, reusing prior artifacts unless `--fresh`.

---

### Artifacts (per run)

Stored under `.adev/runs/<run-id>/` in the target repo:
...
**Git policy:** Spec artifacts (`requirements`, `design`) may be committed to a `specs/<feature-slug>/` path on user approval. Code changes live on branch `adev/<slug>-<run-id>` in an isolated worktree.
...
**Delivery:** Local web UI at `http://localhost:9477` (default port), started via `adev ui` or auto-started on `adev run`.
...
Project declares validation in `.adev/config.yaml`:
...
- Custom agent authoring UI (edit prompt files in `.adev/agents/` instead)
...
1. **As a developer**, I run `adev run "Add dark mode toggle"` and see eight stages progress in the dashboard.
